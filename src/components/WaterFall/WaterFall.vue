...
function onLoad(url: string, i: number) {
  return new Promise((resolve) => {
    ...
    image.onload = function () { // 图片加载完成时执行，此时可通过image.width和image.height获取到图片原始宽高
      let height = image.height / (image.width / imageWidth.value)
...
<template>
  <div v-if="mode==='JS'" v-bind="$attrs" class="m-waterfall-js" ref="waterfall"
  ...
  <div v-if="mode==='CSS'" v-bind="$attrs" class="m-waterfall-css"
       :style="`background: ${backgroundColor}; width: calc(${totalWidth} - ${2 * props.columnGap}px); padding: ${columnGap}px; column-count: ${columnCount}; column-gap: ${columnGap}px;`">
